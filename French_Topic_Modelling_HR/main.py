...
def plot_bokeh(n_top_words=5,plot_width=1600,plot_height=700):
    ...
    for i, k in enumerate(topic_word):
        for t in range(10):
            topic_wrds[k,t] = list(np.array(vocab[k])[np.argsort(topic_word[k])][t][:-(n_top_words + 1):-1])
    
    topic_summry_lst = []
    for key,value in topic_wrds.items():
        topic_summry_lst.append((key[0],value))

    topic_summry_df = pd.DataFrame(topic_summry_lst)
    topic_summs = topic_summry_df.groupby(0)[1].apply(list).to_dict()

    for key,value in topic_summs.items():
        templist = []
        for single in topic_summs[key]:
            templist.append(' '.join(single))
        topic_summs[key] = templist
    ...
    for r1,r2 in enumerate(plot_dict):
        ...
        topic_crd = np.empty((X_topics[r2].shape[1], 2)) * np.nan
        topic_summs2 = {}
        for topic_num in _lda_keys[r2]:
            if not np.isnan(topic_crd).any():
                break
            topic_crd[topic_num] = tsne_lda[r2][_lda_keys[r2].index(topic_num)]
            topic_crd2 = topic_crd[~np.isnan(topic_crd).any(axis=1)]
            nans = np.where(np.isnan(topic_crd))
            nans_uniq = np.unique(nans[0])
            if nans_uniq.size != 0:
                topic_summs2[r2] = [j for i, j in enumerate(topic_summs[r2]) if i not in nans_uniq]
            else:
                topic_summs2[r2] = topic_summs[r2]
        # plot crucial words
        for i in range(len(topic_crd2)):
            plot_lda.text(topic_crd2[i, 0], topic_crd2[i, 1], [topic_summs2[r2][i]])
        # hover tools    
